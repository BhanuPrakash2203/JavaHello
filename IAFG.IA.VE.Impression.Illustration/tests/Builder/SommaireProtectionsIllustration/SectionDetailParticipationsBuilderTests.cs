...
{
    [TestClass]
    public class SectionDetailParticipationsBuilderTests
    {
        private static readonly IFixture Auto = AutoFixtureFactory.Create();
        private SectionDetailParticipationsBuilder _builder;
        ...
        private SectionDetailParticipationsViewModel _sectionViewModel;

        [TestInitialize]
        public void Initialize()
        {
            _sectionViewModel = Auto.Create<SectionDetailParticipationsViewModel>();
            _reportFactoryContainer = Substitute.For<IReportFactory>();
            _context = Auto.Create<IReportContext>();
            _builder = new SectionDetailParticipationsBuilder(_reportFactoryContainer);
        }
        ...
        public void Build_WhenBuidParameters_ShouldAssembleReportProperly()
        {
            ...
            _builder.Build(new BuildParameters<SectionDetailParticipationsViewModel>(_sectionViewModel)
            {
            ...
            _masterReport.Received(1).AddSubReport(Arg.Any<ISectionDetailParticipations>());
        }
    }
}
