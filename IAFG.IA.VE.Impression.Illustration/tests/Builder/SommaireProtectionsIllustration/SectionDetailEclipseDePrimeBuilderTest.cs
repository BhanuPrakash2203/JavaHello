...
{
    [TestClass]
    public class SectionDetailEclipseDePrimeBuilderTest
    {
        private static readonly IFixture Auto = AutoFixtureFactory.Create();
        private ISectionDetailEclipseDePrimeBuilder _builder;
        ...
        private SectionDetailEclipseDePrimeViewModel _sectionViewModel;

        [TestInitialize]
        public void Initialize()
        {
            _sectionViewModel = Auto.Create<SectionDetailEclipseDePrimeViewModel>();
            _reportFactoryContainer = Substitute.For<IReportFactory>();
            _context = Auto.Create<IReportContext>();
            _builder = new SectionDetailEclipseDePrimeBuilder(_reportFactoryContainer);
        }

        [TestMethod]
        public void Build_WhenBuildParameters_ShouldAssembleReportProperly()
        {
            ...
            _builder.Build(new BuildParameters<SectionDetailEclipseDePrimeViewModel>(_sectionViewModel)
            {
            ...
            _masterReport.Received(1).AddSubReport(Arg.Any<ISectionDetailEclipseDePrime>());
        }
    }
}
